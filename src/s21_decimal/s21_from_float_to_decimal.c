...
s21_decimal s21_str_to_decimal(char* str) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        unsigned long long temp_low = (unsigned long long)(unsigned int)result.bits[0] * 10;
        unsigned long long temp_mid = (unsigned long long)(unsigned int)result.bits[1] * 10;
        unsigned long long temp_high = (unsigned long long)(unsigned int)result.bits[2] * 10;

        // прибавляем новую цифру
        temp_low += digit;

        // переносы
        temp_mid += (temp_low >> 32);
        temp_high += (temp_mid >> 32);
...
int s21_from_float_to_decimal(float src, s21_decimal* dst) {
    ...
    if (src == 0.0f) {
        *dst = (s21_decimal){0};
        ...
        return S21_CONVERSION_OK;  // TODO: убрать лишний ретурн
    }

    // Преобразование через строку и парсинг
    ...
    result.bits[3] = (scale << 16) | (sign ? S21_SIGN_NEGATIVE : S21_SIGN_POSITIVE);
