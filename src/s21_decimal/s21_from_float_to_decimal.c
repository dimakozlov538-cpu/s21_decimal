...
s21_decimal s21_str_to_decimal(char* str) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        unsigned long long temp_low = (unsigned long long)(unsigned int)result.bits[0] * 10 + digit;
        unsigned long long temp_mid =
            (unsigned long long)(unsigned int)result.bits[1] * 10 + (temp_low >> 32);
        unsigned long long temp_high =
            (unsigned long long)(unsigned int)result.bits[2] * 10 + (temp_mid >> 32);
