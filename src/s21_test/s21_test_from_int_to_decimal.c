...
static const test_data_t test_data[] = {
    {12345,
     S21_CONVERSION_OK,
     {.bits = {12345, 0, 0, S21_SIGN_POSITIVE}}},  // positive integer
    {-67890,
     S21_CONVERSION_OK,
     {.bits = {67890, 0, 0, S21_SIGN_NEGATIVE}}},  // negative integer
    {0, S21_CONVERSION_OK, {.bits = {0, 0, 0, S21_SIGN_POSITIVE}}},  // zero
    {INT_MAX,
     S21_CONVERSION_OK,
     {.bits = {INT_MAX, 0, 0, S21_SIGN_POSITIVE}}},  // INT_MAX
    {INT_MIN,
     S21_CONVERSION_OK,
     {.bits = {(unsigned int)INT_MAX + 1, 0, 0,
               S21_SIGN_NEGATIVE}}}  // INT_MIN - special case (requires
                                     // unsigned conversion)
};
...
Suite* s21_add_int_to_decimal_suite(void) {
  ...
  tcase_add_loop_test(tc_valid, test_int_to_decimal_valid, 0,
                      sizeof(test_data) / sizeof(test_data[0]));

  TCase* tc_null_pointer =
      tcase_create("s21_from_int_to_decimal: Null pointer");
